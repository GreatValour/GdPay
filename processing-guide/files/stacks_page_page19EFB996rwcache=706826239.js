...
var jQuery = stacks.jQuery;var $ = jQuery;$(document).ready(function(){
	if ($('#stacks_in_5565').length){

		$('#stacks_in_5565 > .nav-bar-pro-spacer > .nav-bar-pro-wrapper').parentsUntil('.stacks_top').css({'overflow' : 'visible'});
		$('.stacks_top').css({'overflow' : 'visible'});

		// Auto detect active (.active) page
		// Site _MUST_ use Tidy Links for this to be used.
		$('#stacks_in_5565 ul.nav-bar-items li a').each(function (index){
		  if ($(this).attr('href') == "./") {
			$(this).addClass('active');
		  }
		});

		$('#stacks_in_5565 ul.nav-bar-drop-down-items li a').each(function (index){
			if ($(this).attr('href') == "./") {
			  $(this).addClass('active');
			}
		});
		...
		//
			...
			closestBanner = $('#stacks_in_5565').closest('.nav_pairing');
			
			$('#stacks_in_5565').prependTo(closestBanner);
			...
			var stickyOffset = $('#stacks_in_5565 .nav-bar-pro-wrapper').offset().top;
			...
			$(window).scroll(function(){
				var sticky = $('#stacks_in_5565 .nav-bar-pro-wrapper'),
					scroll = $(this).scrollTop();

				if (scroll > stickyOffset) sticky.addClass('stuck');
				if (scroll <= stickyOffset) sticky.removeClass('stuck');
			});
		

	
		$('#stacks_in_5565 .nav-bar-drop-down-toggle').click(function() {
			if ($("#stacks_in_5565 .nav-bar-drop-down").is(":visible") == true) { 
				TweenMax.to($('#stacks_in_5565 .nav-bar-drop-down'), 0.20, { delay: 0.05, startAt:{ y:0, x:0, scale: 1, rotation:0, rotationY:0, rotationX:0, opacity: 1 }, y: 30, x: 0, scale: 1.00, rotation: 0, rotationY:0, rotationX: 0, opacity: 0 , ease: Power3.easeOut},0).eventCallback("onComplete", function(){
					$("#stacks_in_5565 .nav-bar-drop-down").css({'opacity' : '0', 'display' : 'none'});
				});
			} else {
				$("#stacks_in_5565 .nav-bar-drop-down").css({'opacity' : '0', 'display' : 'block'});
				TweenMax.to($('#stacks_in_5565 .nav-bar-drop-down'), 0.20, { delay: 0.05, startAt:{ y: 30, x: 0, scale: 1.00, rotation:0, rotationY:0, rotationX: 0, opacity: 0 }, y:0, x:0, scale:1.0, rotation: 0, rotationY:0, rotationX: 0, opacity: 1 , ease: Power3.easeOut},0);
				
				
				TweenMax.staggerFromTo("#stacks_in_5565 .nav-bar-drop-down > ul > li", 0.30, {opacity:0, y: 40, x: 0, scale:1}, {delay: 0.05, opacity:1, y:0, x:0, scale:1, ease: Power3.easeOut}, 0.1);
		...
		$('#stacks_in_5565').click(function(event){
			event.stopPropagation();
		});


		$('#stacks_in_5565 a.drop-down-expand-parent').click(function(event) {
...
var jQuery = stacks.jQuery;var $ = jQuery;/*
 * Table of Contents Generator Stack By WeaverAddons.com
 * Version 2.0.0
 *
...
(function(c){c.TableOfContents=function(i,f,j){var a=this;a.$el=c(i);a.toc="";a.listStyle="ul";a.tags="h1 h2 h3 h4 h5 h6".split(" ");a.levels=[0,0,0,0,0,0];a.uniqueHeadings=[];a.duplicateHeadings=1;a.init=function(){a.options=c.extend({},c.TableOfContents.defaultOptions,j);if("undefined"==typeof f||null==f)f=document.body;a.$scope=c(f);if(1==a.$scope.find(a.tags.join(", ")).filter(":first").length){a.starting_depth=a.options.startLevel;0>a.options.depth&&(a.options.depth=0);var b=a.tags.splice(a.options.startLevel-
1,a.options.depth+1);a.$headings=a.$scope.find(b.join(", "));if(!1!==a.options.topLinks){if("toc"==a.options.topLinkStop)a.topLinkId=a.$el.attr("id");else{b=c(document.body).attr("id");if(""==b||"undefined"==typeof b)b=a.options.topBodyId,document.body.id=b;a.topLinkId=b}a.options.topLinkClassOrig=a.options.topLinkClass;"far_right"==a.options.topLinkMode?a.options.topLinkClass+=" "+a.options.stackID+"-toc-top-link-right":"fixed"==a.options.topLinkMode&&(a.options.topLinkClass+=" "+a.options.stackID+
"-toc-top-link-fixed")}a.buildTOC();!0===a.options.smoothScrolling&&a.smoothScroll();!0===a.options.proportionateSpacing&&!a.tieredList()&&a.addSpacing();if(!1!==a.options.topLinks&&"fixed"==a.options.topLinkMode){var d=a.$el.offset().top+a.$el.height();c(window).scroll(function(){c(this).scrollTop()>d?c("."+a.options.stackID+"-toc-top-link-fixed").fadeIn(500):c("."+a.options.stackID+"-toc-top-link-fixed").fadeOut(500)})}return a}};a.tieredList=function(){return"ul"==a.listStyle||"ol"==a.listStyle};
a.buildTOC=function(){a.current_depth=0;a.options.title&&(a.toc='<div class="toc-header">'+a.options.title+"</div>");a.toc+="<ul>";var b=!1,d=a.$scope.find(".stacks_in:first-child > :first-child")[0];a.$headings.each(function(e,h){var g=this.nodeName.toLowerCase().substr(1,1);(0<e||0==e&&g!=a.current_depth)&&a.changeDepth(g,0==e);a.toc+=a.formatLink(this,g,e)+"\n";"fixed"!=a.options.topLinkMode&&!1!==a.options.topLinks&&(a.options.floatToc&&!b?(b=!0,d!=h&&a.addTopLink(this)):a.options.topLinkStartLevelOnly?
0==a.current_depth&&(a.addTopLink(this),b=!0):(a.addTopLink(this),b=!0));a.options.floatToc&&a.$el.css("margin-top",c(d).css("margin-top"));!1!==a.options.addMarkers&&a.addMarker(this)});"fixed"==a.options.topLinkMode&&!1!==a.options.topLinks&&a.addTopLink();a.changeDepth(a.starting_depth,!0);a.toc+="</ul>";a.$el.html(a.toc);a.$el.find("li:empty").remove();a.$el.children("li").addClass("first-level");if(a.options.floatToc){var e=a.$scope.find(".stacks_in:first");e.height()>a.$el.height()&&e.prepend(a.$el)}};
a.smoothScroll=function(){c(document).ready(function(){function b(a){return a.replace(/^\//,"").replace(/(index|default).[a-zA-Z]{3,4}$/,"").replace(/\/$/,"")}var d=b(location.pathname),e=function(a){for(var b=0,d=arguments.length;b<d;b++){var e=arguments[b],f=c(e);if(0<f.scrollTop())return e;f.scrollTop(1);var i=0<f.scrollTop();f.scrollTop(0);if(i)return e}return c("html, body")}("html","body",document,window),f=/msie [67]/i.test(navigator.userAgent);f&&(e=c("html, body"));a.$el.find("a[href*=#]").add(c("a."+
a.options.topLinkClassOrig)).each(function(){var h=b(this.pathname)||d;if(d==h&&(location.hostname==this.hostname||!this.hostname)&&this.hash.replace(/#/,"")){var h=c(this.hash),g=this.hash;if(g){var i=h.offset().top;e.length&&c(this).click(function(b){b.preventDefault();c(e).animate({scrollTop:i},a.options.scrollSpeed,function(){f||location.replace(g)})})}}})})};a.addMarker=function(b){var d=a.levels.join(a.options.markerSeparator).replace(/(\.0)+$/,"")+a.options.markerSuffix;c(b).prepend('<span class="'+
a.options.markerClass+'">'+d+"</span> ")};a.addTopLink=function(b){var d=a.options.topLinkText?a.options.topLinkText:"Top",d=c("<a href='#"+a.topLinkId+"' class='"+a.options.topLinkClass+"'></a>").html(d);b?"far_right"==a.options.topLinkMode?(c(b).prepend(d),(b=c(b).height()-d.outerHeight(!0))&&d.css("margin-top","-"+Math.abs(b/2)+"px")):c(b).append(" ").append(d):c("body").append(d)};a.formatLink=function(b){var d=b.id,e=c(b).text();""==d&&(d=a.buildSlug(e),b.id=d);b="<a href='#"+d+"'";0<a.options.truncate&&
e.length>a.options.truncate&&(textTruncated=c.trim(e).substring(0,a.options.truncate).split(" ").slice(0,-1).join(" "),textTruncated=textTruncated.replace(/[!-\/:-@\[-`{-~]$/,""),e=textTruncated+"...");return b+=">"+(!a.options.bulletedList?a.levels.join(a.options.markerSeparator).replace(/(\.0)+$/,"")+a.options.markerSuffix+" ":"")+a.options.levelText.replace("%",e)+"</a>"};a.changeDepth=function(b,d){var c=b-a.options.startLevel;!0!==d&&(d=!1);var f=a.tieredList();if(c>a.current_depth){for(var h=
[],g=a.current_depth;g<c;g++)h.push("<"+a.listStyle+">\n"),a.current_depth++,a.levels[a.current_depth+1]=0;c="<li class='"+a.levelClass(b)+"'>\n";f&&(a.toc+=h.join(c)+c)}else if(c<a.current_depth){h=[];for(g=a.current_depth;g>c;g--)h.push("</"+a.listStyle+">\n"),a.levels[a.current_depth]=0,a.current_depth--;f&&(a.toc+="</li>\n"+h.join("</li>\n"),d||(a.toc+="</li>\n<li class='"+a.levelClass(b)+"'>\n"))}else a.levels[a.current_depth+1]=0,f&&(a.toc=d?a.toc+("<li class='"+a.levelClass(b)+"' style='margin-top:0;padding-top:0;'>"):
a.toc+("</li>\n<li class='"+a.levelClass(b)+"'>\n"));a.levels[a.current_depth]++};a.buildSlug=function(b){b=b.toLowerCase().replace(/[^a-z0-9 -]/gi,"").replace(/ /gi,"-");b=b.substr(0,50);-1!=c.inArray(b,a.uniqueHeadings)&&(b+="-"+a.duplicateHeadings,a.duplicateHeadings++);a.uniqueHeadings.push(b);return b};a.levelClass=function(b){return a.options.levelClass.replace("%",b)};a.depthClass=function(b){return a.options.depthClass.replace("%",b-(a.starting_depth-1))};a.addSpacing=function(){var b=a.$headings.filter(":first").position().top;
a.$headings.each(function(d){var d=a.$el.find("a:eq("+d+")"),e=(c(this).position().top-b)/(a.$scope.height()-b)*a.$el.height();d.css({position:"absolute",top:e})})};return a.init()};c.TableOfContents.defaultOptions={startLevel:1,depth:3,levelClass:"toc-level-%",depthClass:"toc-depth-%",levelText:"%",topLinks:!1,topLinkText:"top",topLinkClass:"toc-top-link",topBodyId:"toc-top",proportionateSpacing:!1,smoothScrolling:!0,fixedPosition:!1,title:"Table of Contents",width:"proportionate",autoClose:!0,showOnClick:!0,
addMarkers:!0,markerSeparator:".",markerSuffix:")",markerClass:"",scrollSpeed:400};c.fn.tableOfContents=function(i,f){return this.each(function(){new c.TableOfContents(this,i,f)})}})(jQuery);
$(document).ready(function(){ 
	if (0) {
		var relocation_id = $.trim('');
		var first_character = relocation_id.charAt(0);
        ...
        if (relocation_id.length) {
            $('#stacks_in_2420container').appendTo(relocation_id);
        }
    }
	...
	$("#stacks_in_2420container").tableOfContents(
		$("#stacks_in_2420content"), 
		{
			startLevel: 2,
			depth: depth,
			topLinks: true,
			...
			bulletedList: 'disc' == 'disc',
			floatToc: 'percentage' == 'float',
			topLinkMode: 'true',
			topLinkStop: 'top',
			topLinkStartLevelOnly: 0,
			topLinkClass: 'stacks_in_2420-toc-top-link',
			stackID: 'stacks_in_2420',
			truncate: (0 ? 100 : 0),
			scrollSpeed: 400
		}
	); 
...
stacks.stacks_in_5494_4 = {};
stacks.stacks_in_5494_4 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery;$(document).ready(function(){
...
return stack;})(stacks.stacks_in_5494_4);
stacks.stacks_in_5494_8 = {};
stacks.stacks_in_5494_8 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery;$(document).ready(function(){
	
		$('#stacks_in_5494_8 > .container').parentsUntil('.stacks_top').css('overflow', 'visible');
		$('.stacks_top').css({'overflow' : 'visible'});
	
});

return stack;})(stacks.stacks_in_5494_8);
stacks.stacks_in_5494_47 = {};
stacks.stacks_in_5494_47 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery;$(document).ready(function(){
	
		$('#stacks_in_5494_47 > #animation-element-stacks_in_5494_47').parentsUntil('.stacks_top').css({'overflow' : 'visible'});
		$('.stacks_top').css({'overflow' : 'visible'});
	...
	if (!isMobile) {
		...
		var offsetValue = ($(window).height() * 0.9);
						...
						TweenMax.to($('#animation-element-stacks_in_5494_47'), 1.00, {  delay: 0.50, startAt:{rotation: 0, y:-50, x:0, scale:1.00, opacity:0.00, rotationY:0, rotationX:0}, scale:1, opacity:1.00, y:0, x:0, rotationY:0, rotationX:0, rotation:0, ease: Power3.easeOut }
			);
		...
		$('#animation-element-stacks_in_5494_47').css({'opacity':'1'});

	}

});

return stack;})(stacks.stacks_in_5494_47);
stacks.stacks_in_5494_52 = {};
stacks.stacks_in_5494_52 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery;$(document).ready(function(){
	
		$('#stacks_in_5494_52 > #animation-element-stacks_in_5494_52').parentsUntil('.stacks_top').css({'overflow' : 'visible'});
		$('.stacks_top').css({'overflow' : 'visible'});
	...
	if (!isMobile) {
		...
		var offsetValue = ($(window).height() * 0.9);
						...
						TweenMax.to($('#animation-element-stacks_in_5494_52'), 1.00, {  delay: 1.20, startAt:{rotation: 0, y:-50, x:0, scale:1.00, opacity:0.00, rotationY:0, rotationX:0}, scale:1, opacity:1.00, y:0, x:0, rotationY:0, rotationX:0, rotation:0, ease: Power3.easeOut }
			);
		...
		$('#animation-element-stacks_in_5494_52').css({'opacity':'1'});

	}

});

return stack;})(stacks.stacks_in_5494_52);
stacks.stacks_in_5494_56 = {};
stacks.stacks_in_5494_56 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery;$(document).ready(function(){
	
		$('#stacks_in_5494_56 > #animation-element-stacks_in_5494_56').parentsUntil('.stacks_top').css({'overflow' : 'visible'});
		$('.stacks_top').css({'overflow' : 'visible'});
	...
	if (!isMobile) {
		...
		var offsetValue = ($(window).height() * 0.9);
						...
						TweenMax.to($('#animation-element-stacks_in_5494_56'), 1.00, {  delay: 1.90, startAt:{rotation: 0, y:-50, x:0, scale:1.00, opacity:0.00, rotationY:0, rotationX:0}, scale:1, opacity:1.00, y:0, x:0, rotationY:0, rotationX:0, rotation:0, ease: Power3.easeOut }
			);
		...
		$('#animation-element-stacks_in_5494_56').css({'opacity':'1'});

	}

});

return stack;})(stacks.stacks_in_5494_56);